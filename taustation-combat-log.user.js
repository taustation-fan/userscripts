...
// @downloadURL  https://github.com/taustation-fan/userscripts/raw/master/taustation-combat-log.user.js
// @version      1.0
...
`;
    ...
    function tSCL_show_combat_log_window(show_window) {
        ...
        //debug console.log((show_window ? 'Showing' : 'Hiding') + ' Combat log window.');
    }
...
//
    ...
    function tSCL_process_combat_round(combat_area, random_encounter) {
        ...
        {
            //debug console.log('tSCL_process_combat_round(): Starting / continuing combat.');
            ...
            //debug console.log('tSCL_process_combat_round(): Combat finished.');
            var combat_round = tSCL_prepare_log_entry(combat_log, combat_log_scratch);
            combat_log_scratch = combat_log_scratch.find('div.combat-round:last-of-type');
    ...
    function tSCL_add_combat_details_to_log(combat_area, combat_log_scratch, combat_round) {
        ...
        if (combat_round > 0) {
            ...
            for (var actor in actor_table) {
                ...
                if (weapon_node.length) {
                    ...
                    //debug console.log(actor + ': Found weapon name in game\'s combat actions log.');
                }
                ...
                if (! weapon_name && actor == 'self' &&
                    ...
                    if (uuid) {
                        var weapon_node = combat_area.find('#player-content-' + actor).find('input[value="' + uuid + '"]')
                                                     .parent().find('.combat-slot--desc').clone();
                        if (weapon_node.length) {
                            weapon_name = weapon_node[0].innerText;
                            //debug console.log(actor + ': Found weapon name in page URL.');
                        }
                    }
                }
                ...
                if (! weapon_name) {
                    ...
                    if (weapon_node.length) {
                        ...
                        //debug console.log(actor + ': Found weapon name from checked item in character table.');
                    }
                }

                if (! weapon_name) {
                    weapon_name = "[unknown]";
                    //debug console.log(actor + ': Weapon name not found.');
                }
        ...
        if (any_damage_done ||
            ...
            if (! localStorage[storage_key_prefix + 'combat_saw_player_tables']) {
                ...
                //debug console.log('Did damage; haven\'t seen player tables yet, so not removing anything.');
            } else {
                ...
                //debug console.log('Did damage; already saw player tables, so removing \'class="combat-player--main-content"\' node & children.');
            }
        } else {
            ...
            //debug console.log('No damage done;  removing \'class="combat-player"\' node & children.');
        }
    ...
    function tSCL_clear_log() {
        for (var item in localStorage) {
            if (item.startsWith(storage_key_prefix + "combat_")) {
                //debug console.log('tSCL_clear_log(): Removing localStorage item: ' + item);
